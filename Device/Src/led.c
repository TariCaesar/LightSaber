...
volatile LED_COLOR ledColorData[LED_NUM];
static volatile uint16_t ledColorDataUnroll[LED_NUM * 24];

const uint16_t ledHighData = 66;
const uint16_t ledLowData = 33;
const uint16_t ledResetData[LED_RESET_DATA_LENGTH] = {0};

static volatile int32_t ledIsUpdating = 0;
static volatile int32_t ledUpdateEventCnt = 0;
...
static void LedUpdateEventHandler(){
    ledUpdateEventCnt += 1;
    LedUpdate();
    if(ledUpdateEventCnt == 25){
        ledUpdateEventCnt = 0;
        //refresh bluetooth
...
int32_t LedInit(){
    ...
    ledIsUpdating = 0;
    ledUpdateEventCnt = 0;
    Timer4Init(LedUpdateEventHandler, 23);
    Timer4Enable();
